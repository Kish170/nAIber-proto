...
export class HealthPrompt extends SystemPrompt {
    ...
    async generateFirstMessage(userProfile: UserProfile, openAIClient: OpenAIClient): Promise<string> {
        ...
        const firstCallPrompt = `
            Generate a short, warm opening message for a health check-in call.

            User's name: ${name}
            Time of day: ${partOfDay}
            This is their first health check-in call.
        ...
        const returningCallPrompt = `
            ...
            User's name: ${name}
            Time of day: ${partOfDay}
            This user has had a health check-in before.
        ...
        try {
            const response = await openAIClient.generalGPTCall({
                messages: [
                    {
                        role: 'system',
                        content: isFirstCall ? firstCallPrompt : returningCallPrompt
                    }
                ]
            });
